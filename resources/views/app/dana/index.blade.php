                ...
                <div class="col-12 col-md-12 col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('app.agenda.create') }}" class="btn btn-primary">Tambah {{ $pageTitle }}</a>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered table-md" id="tableDana">
                                    ...
                                    @forelse($danas as $dana)
                                        <tr>
                                            ...
                                            <td>{{ $dana->kategori_dana }}</td>
                                            <td>{{ $dana->transaction_date }}</td>
                                            <td>Rp {{ $dana->amount }}</td>
                                            <td style="display: flex" class="text-center">
                                                <a href="{{ route('app.dana.edit', $dana) }}" data-toggle="tooltip" title="" class="btn btn-info btn-action mr-2" data-original-title="Ubah"><i class="fas fa-pencil-alt"></i></a>
                                                <form action="{{ route('app.dana.destroy', $dana) }}" id="formDelete" method="post">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
